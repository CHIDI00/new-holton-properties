...
interface ApiProjects {
  ...
  status: string;
  description: string;
  type: string; // e.g., 'residential', 'commercial', 'mixed-use'
  bedrooms: number;
  bathrooms: number;
  price: string;
  image_paths: string[];
...
const ProjectContent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        ...
        const projects = Array.isArray(data) && Array.isArray(data[0]) ? data[0] : data;
  ...
  useEffect(() => {
    ...
    if (filters.location) {
      currentFilteredData = currentFilteredData.filter(
        (project) => project.location.toLowerCase() === filters.location.toLowerCase()
      );
    }
