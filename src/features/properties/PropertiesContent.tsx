...
const PropertiesContent: React.FC = () => {
  ...
  const [filteredPropertyData, setFilteredPropertyData] = useState<ApiProperties[]>([]);
  ...
  useEffect(() => {
    const fetchPropertyData = async () => {
      try {
        ...
        const properties = Array.isArray(data) && Array.isArray(data[0]) ? data[0] : data;
    ...
    if (filters.priceRange) {
      ...
      currentFilteredData = currentFilteredData.filter(
        (property) => {
          ...
          return propertyActualPrice >= minPrice && propertyActualPrice <= maxPrice;
        }
      );
    }
