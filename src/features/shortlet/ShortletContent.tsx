...
import FilterForm from "../../ui/FilterForm"; // Adjust this path if FilterForm is elsewhere
...
const parsePriceToNumber = (priceString: string): number => {
  ...
  let cleanedPrice = priceString.replace(/[₦$,\s]/g, '');
...
const ShortletContent: React.FC = () => {
  ...
  const [filteredShortletData, setFilteredShortletData] = useState<ApiShortlet[]>([]);
  ...
  useEffect(() => {
    const fetchShortletData = async () => {
      try {
        ...
        const shortlets = Array.isArray(data) && Array.isArray(data[0]) ? data[0] : data;
  ...
  useEffect(() => {
    ...
    if (filters.location) {
      currentFilteredData = currentFilteredData.filter(
        (shortlet) => shortlet.location.toLowerCase() === filters.location.toLowerCase()
      );
    }
    ...
    if (filters.priceRange) {
      ...
      currentFilteredData = currentFilteredData.filter(
        (shortlet) => {
          ...
          return shortletActualPrice >= minPrice && shortletActualPrice <= maxPrice;
        }
      );
    }
    ...
    return (
      <div>
        ...
        <div className="container h-[40rem] mx-auto md:px-[11rem] px-[2rem] flex justify-center items-center py-20">
          <Loader size={70} className="animate-spin text-gray-500" />
        </div>
      </div>
    );
  }
